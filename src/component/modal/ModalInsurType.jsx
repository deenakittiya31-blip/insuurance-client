...
const ModalInsurType = ({ value, onSubmit, onChange }) => {
    ...
    return (
        <div className='font-prompt'>
            <button className="btn bg-main px-5 rounded-md text-white font-semibold" onClick={() => document.getElementById('modalinsurtype').showModal()}>เพิ่มข้อมูล</button>
            <dialog id="modalinsurtype" className="modal">
                <form onSubmit={onSubmit} className="modal-box w-auto flex flex-col gap-5">
                    <h3 className="font-bold text-lg text-text-primary">เพิ่มข้อมูลประเภทประกัน</h3>
                    <TextInput
                        width='w-sm'
                        title='ประเภทประกัน'
                        name='nametype'
                    ...
                    <div className='modal-action'>
                        <button type='button' className="btn btn-soft btn-error" onClick={() => document.getElementById('modalinsurtype').close()}>ยกเลิก</button>
